...
const TransformationForm = ({ action, data = null, userId, type, creditBalance, config = null }: TransformationFormProps) => {
      ...
      if(action === 'Add') {
        try {
          ...
          if(newImage) {
            form.reset()
            setImage(data)
            router.push(`/tools/image-modifiers/transformations/${newImage._id}`)
          }
        } catch (error) {
          console.log(error);
        }
      ...
      if(action === 'Update') {
        try {
          const updatedImage = await updateImage({
            ...
            userId,
            path: `/tools/image-modifiers/transformations/${data._id}`
          })

          if(updatedImage) {
            router.push(`/tools/image-modifiers/transformations/${updatedImage._id}`)
          }
        } catch (error) {
          console.log(error);
        }
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8  px-4 py-20 md:p-14">
        ...
        <CustomField
          ...
          className="w-full"
          render={({ field }) => <Input {...field} className="rounded-[16px] border-2 border-purple-200/20 shadow-sm shadow-purple-200/15 text-dark-600 disabled:opacity-100 p-16-semibold h-[50px] md:h-[54px] focus-visible:ring-offset-0 px-4 py-3 focus-visible:ring-transparent !important" required/>}
        />
            ...
            {type === 'recolor' && (
              <CustomField 
                ...
                render={({ field }) => (
                  <Input 
                    value={field.value}
                    className="irounded-[16px] border-2 border-purple-200/20 shadow-sm shadow-purple-200/15 text-dark-600 disabled:opacity-100 p-16-semibold h-[50px] md:h-[54px] focus-visible:ring-offset-0 px-4 py-3 focus-visible:ring-transparent !important"
                    onChange={(e) => onInputChangeHandler(
                      'color',
                      e.target.value,
                      'recolor',
